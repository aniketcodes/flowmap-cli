...
def resolve_symbol(
    ...
    if "." in symbol:
        ...
        results = store.search_symbol(base_name, repo_filter=repo_name, file_filter=rel_file, limit=10)
        for r in results:
    ...
    results = store.search_symbol(symbol, repo_filter=repo_name, file_filter=rel_file, limit=5)
    if results:
        return SymbolMatch(result=results[0], source="file_scoped")

    # 3. Global fallback with file preference (suffix path matching)
    results = store.search_symbol(symbol, repo_filter=repo_name, limit=20)
    for r in results:
...
def get_symbol_suggestions(
    ...
    available = store.get_symbols(repo=repo_name, limit=200)
